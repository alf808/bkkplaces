...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        # process only top level tags: "node" and "way"
        # all attributes of node and way should be turned into regular key/value pairs
        # except those attributes specified in CREATED array
        # and the lat and lon attributes which will be included in key "pos" array
        node["id"] = element.get("id")
        node["type"] = element.tag
        node["visible"] = element.get("visible")
        
        # try to create the lat and lon array for key "pos"
        try:
            node["pos"] = [float(element.get("lat")), float(element.get("lon"))]
        except:
            pass
        
        # prepare the key "created" based on CREATED array
        node["created"] = {}        
        for item in CREATED:
            node["created"][item] = element.get(item)
            
        # handle attribute k as specified in instructions above
